...
# 全文がある場合のkey_findings仕様(本文を読めるため、徹底的に厚く・具体的に)
# 各セクションは **見出し**: 形式。Notion側で見出し+段落に展開される。
_KF_FULLTEXT = (
    "**背景・目的**(5-7文): 疾患・臨床課題の説明、これまでのエビデンスとその限界(具体的な先行研究やガイドラインの立場に触れる)、未解決の臨床的疑問、本研究が埋めようとするギャップ、研究の目的と仮説"
    "\\n\\n**方法**(7-10文): 研究デザイン、実施期間・施設・国、対象集団の組み入れ/除外基準と背景特性(n数・年齢・疾患活動性・既治療歴など)、介入または曝露の具体的内容(薬剤・用量・スケジュール等)、対照群、主要評価項目と副次評価項目の定義、統計解析手法(モデル・補正因子・検出力設計)、事前規定されたサブグループ・感度分析"
    "\\n\\n**主要結果**(8-14文): 主要評価項目の結果を必ず具体的数値(イベント率、群間差、HR/OR/RR、95%CI、p値、NNT等)とともに詳細に記述。経時的推移や用量反応があれば含める。本文の図表で示された主要な数値も文章として拾う"
    "\\n\\n**副次評価項目・サブグループ・安全性**(6-9文): 主要な副次評価項目の結果(数値付き)、臨床的に重要なサブグループ解析(効果修飾の有無)、有害事象・重篤な有害事象・中止率・特定の安全性シグナルを具体的に。感度分析の頑健性にも触れる"
    "\\n\\n**結論**(3-4文): 著者の主要な結論と、それを支持または制限する本文中の具体的データ・論拠"
    "\\n\\n**考察・位置づけ**(6-9文): 既存エビデンス(先行RCT/メタ解析/ガイドライン)との一致点・相違点、結果の生物学的・臨床的妥当性、本研究のstrengths(デザイン上の強み)、結果の一般化可能性"
    "\\n\\n**研究の限界**(4-6文): 本文Discussionで著者が認めるlimitationsに加え、評価者が読み取れる方法論的な弱点(バイアス、交絡、追跡期間、外的妥当性、検出力等)とその臨床解釈上の意味"
    "\\n\\n**臨床応用と今後の展望**(5-7文): 日常診療への具体的な含意(どの患者にどう適用しうるか)、診療を変えうるか、今後必要な研究、そして鷹将さん的リサーチクエスチョン(ASUC risk stratification, single-cell/spatial, refractory UC subtyping, biologic combo therapy, ML/AI in IBD, pharmacogenomics)との接続点を具体的に論じる"
)
...
方法の詳細・結果の具体的数値・考察での限界まで踏み込んだ、**長く・詳細で・読み応えのある**日本語解説を作成してください。

# このタスクで特に重視すること
- 出力は省略せず、各セクションを指定された文数までしっかり埋めて、踏み込んだ深い解説にすること。短くまとめようとしない。
- 本文中の具体的な数値(n数、イベント率、HR/OR/RR、95%CI、p値、絶対差、NNT、有害事象率など)を可能な限り多く拾って明記すること。
- 「abstractを読めば分かる」レベルで止めず、本文Methods/Results/Discussionにしか書かれていない詳細・サブ解析・著者の論証・limitationsまで噛み砕いて解説すること。
- 専門医が原著を読まなくても研究の全体像と臨床的意義を深く理解できる水準を目指すこと。
- 読み手は鷹将さん(IBD臨床・研究に従事)。臨床応用と研究的示唆の両面で具体的に。
